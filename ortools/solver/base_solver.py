...
class Solver:
    ...
    def solve_model(self, settings: SolverSetting):
        ...
        search_parameters.log_search = settings["log_search"]
    ...
    def create_model_with_local_tw(self,
                                   set_max_travel_time=False, tub=1200,
                                   set_max_travel_diff=False, dub=500,
                                   ):
        """
        Create vehicle routing model for Solomon instance.
        """
        ...
        # Add Time Windows constraint.
        tub = tub * self.time_precision_scaler if set_max_travel_time else 10 ** 10
        self.routing.AddDimension(
            transit_callback_index,
            10 ** 10,  # allow waiting time at nodes
            tub,  # maximum time per vehicle route  # change this from 10 ** 10 to 1200 * self.time_precision_scaler will make it hard to find optimal solution
            False,  # Don't force start cumul to zero, i.e. vehicles can start after time 0 from depot
            "Time",
        )
        ...
        # # local time window constraints
        # for node_index in range(self.routing.Size()):
        #     if self.routing.IsEnd(node_index) or self.routing.IsStart(node_index):
        #         continue
        #     current_time = time_dimension.CumulVar(node_index)
        #     next_node_index = self.routing.NextVar(node_index)
        #     next_time = time_dimension.CumulVar(next_node_index)
        #     self.routing.solver().Add(next_time - current_time <= ub)
        #     self.routing.solver().Add(next_time - current_time >= lb)

        # max travel time constraints
        if set_max_travel_diff:
            dub = dub * self.time_precision_scaler
            for j, k in zip(range(self.data["num_vehicles"]), range(self.data["num_vehicles"])):
                if j != k:
                    self.routing.solver().Add(
                        time_dimension.CumulVar(self.routing.End(j)) - time_dimension.CumulVar(self.routing.Start(j))
                        - (time_dimension.CumulVar(self.routing.End(k)) - time_dimension.CumulVar(self.routing.Start(k)))
                        <= dub
                    )
