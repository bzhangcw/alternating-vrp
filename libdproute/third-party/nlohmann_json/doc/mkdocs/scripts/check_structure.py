...
def check_structure():
    ...
    required_headers = ["Examples", "Version history"]
    ...
    for file in files:
        with open(file) as file_content:
            ...
            for lineno, line in enumerate(file_content.readlines()):
                ...
                if h1sections > 1:
                    print(
                        f'{file}:{lineno+1}: Error: unexpected top-level title "{line}"!'
                    )
                    h1sections = 1
                ...
                if line == "# Overview":
                    print(
                        f"{file}:{lineno+1}: Error: overview pages should have a better title!"
                    )

                # lines longer than 160 characters are bad (unless they are tables)
                if len(line) > 160 and "|" not in line:
                    print(
                        f"{file}:{lineno+1}: Error: line is too long ({len(line)} vs. 160 chars)!"
                    )
                ...
                if line.startswith("## "):
                    ...
                    if header in expected_headers:
                        ...
                        if idx <= header_idx:
                            print(
                                f'{file}:{lineno+1}: Error: header "{header}" is in an unexpected order (should be before "{expected_headers[header_idx]}")!'
                            )
                        header_idx = idx
                    else:
                        print(
                            f'{file}:{lineno+1}: Error: header "{header}" is not part of the expected headers!'
                        )
                ...
                if in_initial_code_example and line.startswith("//"):
                    if any(map(str.isdigit, line)) and "(" not in line:
                        print(
                            f"{file}:{lineno+1}: Number should be in parentheses: {line}"
                        )
                ...
                if line == "" and previous_line == "":
                    print(
                        f"{file}:{lineno}-{lineno+1}: Error: Consecutive blank lines!"
                    )
            ...
            for required_header in required_headers:
                if required_header not in existing_headers:
                    print(
                        f'{file}:{lineno+1}: Error: required header "{required_header}" was not found!'
                    )
...
def check_examples():
    ...
    for example_file in example_files:
        ...
        if not found:
            print(
                f"{example_file}: Error: example file is not used in any documentation file!"
            )
